...
router.get('/:db/_all_docs', auth.isAuthenticated, (req, res) => {
  
  if (req.query.keys) {
    
    // add ownerid to requested ids
    req.query.keys = JSON.parse(req.query.keys);
    req.query.keys = req.query.keys.map(id => access.addOwnerId(id, req.session.user.name));
...
router.post('/:db/_all_docs', auth.isAuthenticated, (req, res) => {

  if (req.body.keys) {

    // add ownerid to requested ids
    const body = {};
    body.keys = req.body.keys.map(id => access.addOwnerId(id, req.session.user.name));
